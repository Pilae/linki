...
(async () => {
    ...
    for (const [failure, header] of [[false, '١٬٠٠٢ connections'], [false, null], [true, '١٬٠٠٢ connections']]) {
        ...
        load('lib/withdrawals/store').initWithdrawals(db);
        let pages = 0;
        ...
        m.require = n => n === '@/lib/withdrawals/lock' ? load('lib/withdrawals/lock') : n === '@/lib/linkedin/counts' ? require('./load-linkedin.cjs').loadLinkedIn('counts') : n === '@/lib/db' ? { getDb: () => db } : n === '@/lib/linkedin/session' ? { getSessionPage: async () => page, saveSessionState: async () => { }, markNeedsReauth: async () => { } } : require(n);
