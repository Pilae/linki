...
test('opens authenticated feed and reads through its page, with no credential logging',async()=>{
 ...
 const reader=new SessionReader(page,{conversations:'messengerConversations.'+'a'.repeat(32),messages:'messengerMessages.'+'b'.repeat(32)});
...
test('expired session, redirects, rate limits and malformed responses have safe errors',async()=>{
 ...
 await assert.rejects(new SessionReader(login,{conversations:'messengerConversations.'+'a'.repeat(32),messages:'messengerMessages.'+'b'.repeat(32)}).identity(),e=>e.code==='authentication');
});
