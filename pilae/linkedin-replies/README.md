...
- The PR #8 reader build is installed on the local Linki test server at localhost:3456 with `PILAE_REPLY_ACCOUNT_IDS` empty. The previous login-session build is retained as a stopped rollback container, and the database was backed up. The reply page loads, but no live check has run. Existing Twenty and adapter containers were not deployed or restarted.
