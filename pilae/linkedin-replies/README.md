...
This is an independent Pilae extension, disabled by default. It stores replies and campaign evidence without sending messages, creating runs, or restarting outreach. It is fixture-tested and one authorized local account-scoped synchronization detected and persisted the Quentin campaign reply. The same run projected the event into the local Twenty sandbox. The inbox listing exposed only partial coverage, so `last_success` correctly remains unset; scheduled polling and production deployment remain disabled. Do not enable accounts until the operator explicitly approves reading their conversations.
...
- A local-only image assembled from PR #8 commit `a38c3fb` plus PR #10's session fix at `23c3337` is installed at localhost:3456 with `PILAE_REPLY_ACCOUNT_IDS` empty, so the scheduler cannot poll. Backups and the previous container are retained. A manually invoked, serialized synchronization used temporary process configuration for the one approved account; it did not enable recurring polling.

The public [messaging endpoint reference](https://github.com/vicnaum/linkedin-toolkit/blob/main/references/endpoints.md) informed the experimental GraphQL envelope. The public [mautrix LinkedIn history API](https://github.com/mautrix/linkedin/blob/v0.2608.0/pkg/linkedingo/messages.go) and [query identifiers](https://github.com/mautrix/linkedin/blob/v0.2608.0/pkg/linkedingo/constants.go) identified anchor-timestamp and previous-cursor requests. Mautrix is AGPL-licensed; no implementation was copied. These are protocol leads, not authority to assume our account behaves the same. Bounded Quentin-only reads verified a conversation envelope, top-level participant identities, embedded latest messages, unquoted CSRF header behavior, and full backward traversal of this two-message conversation: inbound anchor page, older outbound page, then an empty `elements` page with `prevCursor: null`. This does not validate longer histories or account-wide listing. See [validation evidence](VALIDATION.md). Missing or changed shapes fail as incomplete instead of reporting an empty inbox. Linki's existing Sustainable Use License remains in force; this work grants no additional hosting rights.
...
Sync-only snapshots with `newSyncToken` retain embedded messages and a separate opaque checkpoint, but remain incomplete: a sync token is not a historical-page cursor. Historical scans start at a timestamp anchor and follow `prevCursor` backward. Only an explicit null previous cursor marks the conversation history complete; unknown or empty pages with a non-null cursor remain incomplete. Coverage gaps persist across interrupted cycles; they never advance `last_success`. Incremental sync-token replay is not implemented.
...
Companion changes are in `../pilae-linki-app/adapter/linkedin-replies/`. Its ingestion reads stored events only, checkpoints pages transactionally, retains unmapped events, maps exact Linki target IDs to Twenty People, and reuses the existing channel-aware reducer/timeline projection. It neither polls LinkedIn nor copies provider logic. Manual overrides, protected statuses, disabled automation and email state remain authoritative. Local tests cover those rules. The local adapter was backed up and restarted with the companion code for the authorized test; it consumed exactly one stable reply event, mapped it to Quentin's existing Person, set the channel-aware inbound timestamp and `REPLIED` status, and created the timeline event. This was a local sandbox validation, not a production deployment.
...
Enabling `PILAE_REPLY_ACCOUNT_IDS` authorizes the server to read those accounts as soon as it starts. Leave it empty until explicit approval. `PILAE_REPLY_CONVERSATIONS_QUERY`, `PILAE_REPLY_HISTORY_ANCHOR_QUERY` and `PILAE_REPLY_HISTORY_PREVIOUS_QUERY` must contain current query IDs of the form `messengerConversations.<32 hex>` and `messengerMessages.<32 hex>` for the latter two. No guessed/default query hashes are shipped. The authorized test read a partial 20-conversation listing and followed the queued histories under repeated 20-page budgets, but the listing supplied a sync snapshot rather than proof of a complete inbox. Repeated polling, incremental sync-token replay, active-campaign stop timing, expired-session recovery, unsupported scopes and other accounts remain unvalidated. Missing configuration or incomplete coverage is reported explicitly.
