...
- A local-only image combining PR #8 with experimental PR #10 login verification is installed at localhost:3456 with `PILAE_REPLY_ACCOUNT_IDS` empty. The PR #8-only image and earlier login-session build are retained as stopped rollback containers, and the database was backed up. The reply page loads, but no live check has run. Existing Twenty and adapter containers were not deployed or restarted.
